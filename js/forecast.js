...
function renderForecast(items = forecastData) {
  ...
  $("#forecast-list").innerHTML = data.map((item) => `
    <article class="forecast-card glass-card" style="cursor: pointer; user-select: none;" title="點擊切換時段">
...
async function loadForecastData() {
  ...
  try {
    ...
    //資料整理好，存入變數，渲染畫面
    forecastData = normalizeForecast(data.records || {});
    renderForecast(forecastData);
    
  } catch (error) {
    console.error("預報資料載入失敗:", error);
    renderForecast([]); //失敗時清空畫面顯示錯誤
  }
}
...
//重新整理按鈕
$("#refresh-button").addEventListener("click", () => loadForecastData());
document.addEventListener("click", (event) => {
  if (event.target.closest("#forecast .retry-data")) loadForecastData();
});

//切換溫度資料
$("#forecast-list").addEventListener("click", (e)=>{
    ...
    periods.forEach((p, i)=>{
        if(p.style.display === "block") activateIndex = i;
    });
    periods[activateIndex].style.display = "none";
    const nextIndex = (activateIndex + 1) % periods.length;
